...
if($idsArchivos!=''){
	...
	for($i=0;isset($vIdArchivo[$i])==true;$i++){
		$sql="select * from archivos where id_sistema=".$tra['id_sistema']." and id_archivo=".$vIdArchivo[$i];
		$db->set_query($sql);
		$db->execute_query();
		$row=$db->get_array();
		if($i==0){
...
$sql="select id_archivo_categoria, archivo_categoria from archivo_categorias where id_sistema=".$tra['id_sistema']." order by archivo_categoria";
$db->set_query($sql);
$db->execute_query();
