...
if($accionRPC=='listarUsuarios'){
	...
	$c++;	$listaUsuarios[0][$c]['html']='ultima acci�n';	$listaUsuarios[0][$c]['width']=100;
...
if($accionRPC=='eliminarUsuario'){
	...
	if($row['fecha_ultima_accion']!=''){
		$resp['mensaje']='No puede eliminar un usuario que ya ha iniciado sesi�n';
	}else{
...
if($accionRPC=='eliminarPermiso'){
	$permiso=recibir_dato('permiso');
	
	$sql="delete from usuario_permisos where permiso='$permiso'";
	$db->set_query($sql);
	$db->execute_query();
