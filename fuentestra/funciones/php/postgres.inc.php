...
/*
echo "print tra in postgres.inc.php: ";
print_r($tra);
*/

//echo "postgres.inc.php " . date("H:i:s:u") . "<br/>" ;

//echo "postgres.inc.php" . "<br/>" ;
...
class db {
  ...
  function connect() {
		...
		//@$this->conection = pg_connect("host=$this->host user=$this->user password=$this->pass dbname=$this->dbname");
		//$connect_sql = "host=$this->host user=$this->user password=$this->pass dbname=$this->dbname port=$this->port options='--client_encoding=UTF8'";
		@$this->conection = pg_connect("host=$this->host port=$this->port dbname=$this->dbname user=$this->user password=$this->pass options='--client_encoding=UTF8'");

		if(!$this->conection) {
			if($bandConectOtraBd==1){
				$resp['estadoConnect'] = "Error al tratar de realizar la conexión a la base de datos, verifique los datos de conección a la base de datos de la versión anterior del sistema <br><br>host=$this->host user=$this->user password=$this->pass dbname=$this->dbname";
			}else{
				echo "Error al tratar de realizar la conexión a la base de datos<br>Error: <br>";
			}
			$this->connected = 0;
			return 1;
		}else{
