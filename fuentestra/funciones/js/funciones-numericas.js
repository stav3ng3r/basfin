function aNumero(cadena){
	...
	var bDec = false;
	for(m=0;m<cadena.length && bDec==false;m++){
		if(cadena.charAt(m) == "."){
			bDec = true;
		}
	}
	...
	if(bDec == true){
		...
		var valor = parseInt(cadena);	
	}
	
    return valor;
}

function aDecimal(cadena){
    cadena=replaceAll(cadena,'.','');
    cadena=replaceAll(cadena,',','.');
    if(cadena==''){cadena=0;}
    var valor = parseFloat(cadena);
...
function initInputSeparadorMiles(){
	$('.inputMiles').number(true,0,',','.');
	$('.inputDecimal').number(true,2,',','.');
}