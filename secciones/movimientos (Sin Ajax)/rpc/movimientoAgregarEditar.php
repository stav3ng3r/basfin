...
if(empty($idMovimientoEdit)){
	...
	$resp['trNuevo']=rpcSeccion('movimientos','movimientosListar');
	
	//echo '<pre>'. $resp['trNuevo'] .'</pre>';
}else{
...
$sql="
	select
		case when sum(md.egreso) is null then sum(md.ingreso) else sum(md.egreso) end as valor,
		p.persona
	from
		sistema.movimiento_detalles md
		join sistema.movimientos m using (id_movimiento)
		join sistema.personas p using (id_persona)
	where
		m.id_movimiento=$idMovimiento
	group by
		p.persona	
";
$db->set_query($sql);
...
$resp['id']=$idMovimiento;
$resp['fecha']=trim($fecha,"'");
$resp['persona']=$row['persona'];
$resp['comprobante_nro']=$comprobanteNro;
$resp['valor']=number_format($row['valor'],0,',','.');	
